// Firebase config & initialization (same as before)
...
async function loadFirestoreHistory(userId) {
    try {
        const docRef = db.collection("chatHistories").doc(userId);
        ...
        if (doc.exists) {
            ...
            chatBox.scrollTop = chatBox.scrollHeight;
            saveLocalHistory(); // also update localStorage after firestore load
        }
    } catch (error) {
        console.error("Error loading Firestore chat history:", error);
    }
}
...
async function saveFirestoreHistory(userId) {
    try {
        const docRef = db.collection("chatHistories").doc(userId);
        await docRef.set({ messages: chatHistory });
    } catch (error) {
        console.error("Error saving Firestore chat history:", error);
    }
}
...
async function sendMessage() {
    ...
    try {
        // Send FULL chat history + new user message as context
        // Format messages in OpenAI style for context understanding
...
Use short messages and send them in chunks if needed, unless user is typing.
        ...
        // Append current user message (just sent)
        messagesForApi.push({
            role: "user",
            content: userText,
        });
...
// PDF reading function unchanged
...
// Firebase auth state listener loads history
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (!loadedLocal) {
            // If no local, load from Firestore
